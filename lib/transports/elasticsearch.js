...
class elasticsearch {
  ...
  setData (data, limit, offset, callback) {
    ...
    data.forEach(elem => {
      const actionMeta = { update: {} }

      // use index from base otherwise fallback to elem
      actionMeta.update._index = this.base.index || elem._index

      // https://www.elastic.co/guide/en/elasticsearch/reference/master/removal-of-types.html
      if (this.ESversion < 7) {
        // use type from base otherwise fallback to elem
        actionMeta.update._type = this.base.type || elem._type
      }
      actionMeta.update._id = elem._id

      if (this.parent.options.handleVersion) {
        if (elem.version || elem._version) {
          actionMeta.update.version = elem.version || elem._version
        }

        if (this.parent.options.versionType) {
          actionMeta.update.version_type = this.parent.options.versionType
        }
      }

      elem.doc_as_upsert = true
...
`
      ...
      payload.body += `${jsonParser.stringify(update, this.parent)}
`
    })
